...
export const classes: { [id: string]: Section } = {};
...
const rows = readFileSync(process.cwd() + '/util/unitime/classes.csv').toString().split(',\n');
...
for (const r of rows.slice(1, rows.length - 1)) {
    ...
    if (type === 'Midterm Examination') {
        ...
        if (sections[0] === 'Offering') {
            const objs = Object.values(classes).filter(s => s.type.includes('Lecture') && names.some(n => s.names.includes(n)));
            if (!objs.length) {
            ...
            if (!classes[sections[0]]) {
                console.log(`[ERR] Midterm for ${names[0]} parsed before corresponding class!`);
                continue;
            }

            const group = classes[sections[0]].midterms.find(g => g[0].date === first);
            if (group) {
                group.push(midterm)
            } else {
                classes[sections[0]].midterms.push([midterm]);
                classes[sections[0]].midterms.sort((g1, g2) => g1[0].date.localeCompare(g2[0].date))
            }
        }
        continue;
    }
    ...
    if (type === 'Final Examination') {
        ...
        if (sections[0] === 'Offering' || sections[0] === 'Course') {
            ...
            classes[sections[0]].finals.push(final);
        }
        continue;
    }
